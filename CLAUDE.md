...
Desktop application for hierarchical management, preview, and export of PDF documents and receipts. Platform: Windows. UI: **React + Vite SPA inside a pywebview host** (Edge WebView2). Version: **3.8.0**.
...
| `version_info.py` | `APP_NAME`, `VERSION` (currently 3.8.0) |
...
Current tag: **v3.8.0** (beta prerelease)
...
- **Version `3.8.0`** (bug form default + BETA_TESTING heading) → bump when `version_info.py` changes.
